...
export default function ImportTextModal({
  ...
  const docLabel = isLetter ? "votre lettre" : "votre CV";
  ...
  const run = async () => {
    ...
    if (!content) {
      toast(`Collez d'abord le contenu de ${docLabel}.`, "error");
      return;
    }
    if (
      !(await uiConfirm(
    ...
    <div className="ui-overlay" role="presentation" onClick={busy ? undefined : onClose}>
      ...
      >
        ...
        <p className="import-hint">
          Collez le contenu brut de {docLabel} : l&apos;IA en extrait les données.
        </p>

        <textarea
          className="form-textarea"
          rows={10}
          placeholder={`Collez ici le texte de ${docLabel}…`}
          value={text}
