...
| **Total** | **14** | Égalité de total avec l'idée n°3 (LinkedIn) tranchée en sa faveur par Apport (4 contre 3) et Cohérence (4 contre 3) : un blocage total et actuel d'un parcours central pour un segment de candidats prime sur un canal (le profil LinkedIn) entièrement fonctionnel aujourd'hui, seulement non couvert par le produit. Reste derrière l'idée n°1 (choix explicite à l'arrivée), qui la bat par Cohérence (5 contre 4) : elle touche la priorité n°1 de `MISSION.md`, ce que ne fait aucune autre idée à 14. |

### 3. Optimisation de profil LinkedIn (analyse + suggestions) — 14/20
...
### 4. Autofill sur d'autres ATS (Workday, iCIMS, SmartRecruiters, Taleo, LinkedIn Easy Apply) — 13/20 — *estimation Facilité peu fiable*
...
### 5. `/pack` : dédupliquer `/api/extract-meta` et sauter l'appel quand l'entreprise/le poste sont déjà connus — 13/20
...
| **Total** | **13** | Égalité avec l'idée n°4 tranchée en sa faveur par Écart (4 contre 1) : l'autofill sert davantage la promesse par une capacité comparée à la concurrence, celle-ci corrige un défaut interne. |

### 6. Remplacer le texte blanc par le jeton `--on-orange` sur les boutons d'action orange (`--cta-grad`) — 13/20
...
| **Total** | **13** | Égalité de total avec les idées n°4 (autofill), n°5 (extract-meta) et n°6 (cta-grad), déjà classées entre elles lors de rounds précédents avec un ordre déjà justifié (voir leurs propres justifications) : aucun des quatre profils n'étant identique, et aucune raison objective ne poussant celle-ci devant les trois autres déjà ancrées, elle est ajoutée en dernière position du groupe à 13, immédiatement avant l'idée n°8 (négociation salariale) qu'elle précède par Cohérence (4 contre 3), au même titre que les trois idées qui la précèdent déjà pour cette même raison. |

### 8. Assistant de négociation salariale — 13/20
...
| Facilité | 4 | Réutiliserait probablement l'infra IA existante (nouveau prompt, pas de nouvelle intégration externe), sur le modèle de l'idée LinkedIn (n°3) — reste à 4 et pas 5 car aucune spec n'a encore été écrite ni relue contre le code, contrairement à celle-ci. |
...
| **Total** | **12** | Égalité avec l'idée n°13 (skill gap) tranchée en faveur de celle-ci par Cohérence (5 contre 4) : la règle de tranchage de `MISSION.md` s'applique ici au sens littéral (facteur ~3,7 mesuré), ce qui n'est le cas d'aucune autre idée du classement à ce jour. Recoupe partiellement l'idée n°17 (« Alléger `/` ») sans la remplacer : celle-ci vise les modales d'import, le dépassement mesuré ici vient des chunks de l'aperçu PDF (police + PDF.js) — deux causes distinctes sur la même route. |
...
| Cohérence | 4 | Touche le seuil explicite « contrastes AA » de `MISSION.md`, sur le panneau ATS, l'un des outils les plus centraux du parcours candidature — même terrain que l'idée n°6 (contraste du texte sur `--cta-grad`), ici sur un fond plutôt qu'un texte. |
| **Total** | **12** | Profil identique (Apport 3, Facilité 4, Écart 1, Cohérence 4) aux idées n°10 (focus clavier) et n°11 (ATS de l'entreprise détecté) : aucune des quatre notes ne les différencie. Conservée juste après elles, sans raison objective de l'y faire passer devant. Passe devant l'idée n°13 (skill gap) et l'idée n°14 (traduction) par Facilité (4 contre 3 chacune) : les trois occurrences à corriger sont déjà localisées précisément dans le code, contre une estimation encore incertaine pour les deux autres. |

### 13. Identification des compétences manquantes vs une offre (« skill gap ») — 12/20 — *estimation Facilité peu fiable*
...
### 14. Traduction du CV/lettre dans une autre langue — 12/20 — *estimation Facilité peu fiable*
...
| **Total** | **12** | Égalité de total et de profil complet (3/3/2/4) avec l'idée n°13 (skill gap) — aucune des deux notes ne les différencie sur le barème. Conservée juste après elle, faute de raison objective de l'y faire passer devant, sur le même principe déjà appliqué à l'idée n°19 (filtre Cadre/Non-cadre) le 03/08/2026. |

### 15. Sortir « Importer un texte » du sous-onglet « Importer » de « Mode Expert » — 12/20
...
| **Total** | **12** | Égalité avec les idées n°9 (aperçu PDF), n°10 (focus clavier), n°11 (ATS de l'entreprise détecté), n°12 (ats-mid hors thème), n°13 (skill gap) et n°14 (traduction), toutes à 12 : Apport le plus bas des sept (2, contre 3 pour les six autres) faute d'un vrai blocage — les six autres décrivent chacune une gêne ou un dépassement réel, celle-ci une simple perte de temps de découverte. Placée en dernière position du groupe à 12. |

### 16. Surligner les mots-clés en contexte, dans l'offre et dans le CV — 11/20
...
### 18. Robustesse du scan face à une offre malformée — 11/20
...
### 19. Câbler ou retirer le filtre « Cadre / Non-cadre » (`QUALIFICATION_OPTIONS`) — 11/20
...
| **Total** | **11** | Égalité de total avec l'idée « Robustesse du scan » (n°18) — profil identique sur les quatre critères (Apport 3, Facilité 4, Écart 1, Cohérence 3). Conservée juste après elle : aucune raison objective identifiée de l'y faire passer devant. |

### 20. Performance `/pack` — mesurer le vrai temps d'interactivité (Monaco/react-pdf) — 11/20
...
### 21. Assombrir `--faint` (texte secondaire) jusqu'au seuil AA dans les deux thèmes — 11/20
...
| **Total** | **11** | Égalité totale de profil (Apport 2, Facilité 5, Écart 1, Cohérence 3) avec l'idée n°20 (perf `/pack`) — aucune des quatre notes ne les différencie. Conservée juste après elle, sans raison objective de l'y faire passer devant. |

### 22. `/api/editor-chat` : élaguer l'historique et ne pas répéter un `doc_json` inchangé — 10/20
...
### 23. Correction orthographique/grammaticale dédiée avec rapport — 10/20 — *estimation Facilité peu fiable*
...
| Facilité | 3 | Chevauchement probable avec le panneau ATS existant (`AtsPanel.tsx`, axe « Structure ») jamais vérifié — pourrait être une extension d'un panneau existant plutôt qu'un nouveau composant. **Estimation peu fiable** tant que ce chevauchement n'est pas confirmé ou infirmé, même profil d'incertitude que l'idée « skill gap » (n°13). |
...
| **Total** | **10** | Égalité avec les idées n°22 (élaguer editor-chat) et n°25 (journal de candidature) tranchée par Facilité : l'idée n°22 est entièrement spécifiée par son constat (Facilité 4) contre une estimation encore incertaine ici, elle passe donc devant. Placée avant l'idée n°25 (Cohérence 3 contre 2) au même titre que la n°22. |

### 24. Introduire des jetons `--radius-sm/md/lg/pill` et remplacer les 19 valeurs de `border-radius` en dur — 10/20
...
| **Total** | **10** | Même profil (Apport 2, Facilité 3, Écart 2, Cohérence 3) que l'idée n°23 (correction orthographique) : aucune des quatre notes ne les différencie. Conservée juste après elle, sans raison objective de l'y faire passer devant. Passe devant l'idée n°25 (journal de candidature, Apport 2, Facilité 4, Écart 2, Cohérence 2) par Cohérence (3 contre 2), au même titre que l'idée n°22 (élaguer editor-chat) et l'idée n°23 déjà classées devant elle pour cette même raison : une Facilité plus faible pèse moins que servir un seuil `MISSION.md` plutôt que d'être en tension avec un principe déjà établi du produit. |

### 25. Journal de candidature (réalisations, culture d'entreprise, questions à poser) — 10/20
...
### 26. Relecture de CV par un humain (service payant) — 9/20 — *sensible : touche
...
### 28. Introduire une échelle `--font-size-*` et remplacer les 20 valeurs de `font-size` en dur — 8/20

Constat détaillé : `boucle/constats/2026-08-07-coherence-visuelle.md` §2.

| Critère | Note | Justification |
|---|---|---|
| Apport | 2 | Comme pour les rayons (idée n°24), aucune couleur ni contraste en jeu — cohérence visuelle pure. Signal aggravant : six des vingt valeurs sont au demi-pixel (9.5/10.5/11.5/12.5/13.5/14.5px), un pas qu'aucune échelle typographique voulue ne produit — plus probablement des ajustements ponctuels non documentés qu'un choix de design, mais reste un défaut de finition plutôt qu'un blocage. |
| Facilité | 2 | Vingt valeurs à répartir en six paliers, dans un fichier de 2896 lignes — surface plus large que les rayons (idée n°24, 19 valeurs en 4 paliers) et risque plus élevé : une taille de texte modifiée peut casser un retour à la ligne ou un alignement là où un rayon modifié ne change que l'angle d'un coin. Aucune mesure de régression visuelle automatisée dans le dépôt — vérification composant par composant à la main. |
...
### 30. Alertes sur de nouvelles offres correspondant au profil — 8/20 — *sensible : suppose
...
### 31. Décider du sort du chargement de Monaco depuis `cdn.jsdelivr.net` — 8/20
...
### 32. Supprimer les fonctions mortes de `db.ts` et `completeJson` de `clients.ts` — 8/20
...
### 33. Capture d'offre native dans l'extension navigateur (éviter le copier-coller d'URL) — 8/20 — *estimation Facilité peu fiable*
...
### 34. Générateur de lettre de démission — 8/20
...
### 35. Compter/plafonner les appels IA comme les job-boards — 8/20 — *sensible : touche
...
| **Total** | **7** | Égalité de total avec l'idée n°37 (remplacer les sélecteurs CSS par `@mozilla/readability`) tranchée en sa faveur par Écart (2 contre 1) : celle-ci correspond à une capacité réellement comparée à la concurrence chez 2 produits sur 8, l'autre concerne un détail d'implémentation serveur invisible depuis l'extérieur d'un produit concurrent, le constat le dit lui-même « sans objet pour ce domaine ». Signalée explicitement sensible : sortir du 100 % local est un choix d'architecture, à trancher par le propriétaire. |

### 37. Remplacer les sélecteurs CSS faits main de `scrapeJobText` par `@mozilla/readability` — 7/20 — *sensible : ajout d'une dépendance npm importante, feu vert requis ; estimation Facilité peu fiable*
