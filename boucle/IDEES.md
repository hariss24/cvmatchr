...
| **Total** | **12** | Égalité avec l'idée n°15 (skill gap) tranchée en faveur de celle-ci par Cohérence (5 contre 4) : la règle de tranchage de `MISSION.md` s'applique ici au sens littéral (facteur ~3,7 mesuré), ce qui n'est le cas d'aucune autre idée du classement à ce jour. Recoupe partiellement l'idée n°19 (« Alléger `/` ») sans la remplacer : celle-ci vise les modales d'import, le dépassement mesuré ici vient des chunks de l'aperçu PDF (police + PDF.js) — deux causes distinctes sur la même route. |
...
### 18. Surligner les mots-clés en contexte, dans l'offre et dans le CV — 11/20
...
### 20. Robustesse du scan face à une offre malformée — 11/20
...
### 21. Câbler ou retirer le filtre « Cadre / Non-cadre » (`QUALIFICATION_OPTIONS`) — 11/20
...
| **Total** | **11** | Égalité de total avec l'idée « Robustesse du scan » (n°20) — profil identique sur les quatre critères (Apport 3, Facilité 4, Écart 1, Cohérence 3). Conservée juste après elle : aucune raison objective identifiée de l'y faire passer devant. |

### 22. Performance `/pack` — mesurer le vrai temps d'interactivité (Monaco/react-pdf) — 11/20
...
### 23. Assombrir `--faint` (texte secondaire) jusqu'au seuil AA dans les deux thèmes — 11/20
...
| **Total** | **11** | Égalité totale de profil (Apport 2, Facilité 5, Écart 1, Cohérence 3) avec l'idée n°22 (perf `/pack`) — aucune des quatre notes ne les différencie. Conservée juste après elle, sans raison objective de l'y faire passer devant. |

### 24. Fermer le contournement par DNS rebinding de la protection SSRF existante — 10/20
...
### 25. `/api/editor-chat` : élaguer l'historique et ne pas répéter un `doc_json` inchangé — 10/20
...
### 26. Correction orthographique/grammaticale dédiée avec rapport — 10/20 — *estimation Facilité peu fiable*
...
| **Total** | **10** | Égalité avec les idées n°25 (élaguer editor-chat) et n°28 (journal de candidature) tranchée par Facilité : l'idée n°25 est entièrement spécifiée par son constat (Facilité 4) contre une estimation encore incertaine ici, elle passe donc devant. Placée avant l'idée n°28 (Cohérence 3 contre 2) au même titre que la n°25. |

### 27. Introduire des jetons `--radius-sm/md/lg/pill` et remplacer les 19 valeurs de `border-radius` en dur — 10/20
...
| **Total** | **10** | Même profil (Apport 2, Facilité 3, Écart 2, Cohérence 3) que l'idée n°26 (correction orthographique) : aucune des quatre notes ne les différencie. Conservée juste après elle, sans raison objective de l'y faire passer devant. Passe devant l'idée n°28 (journal de candidature, Apport 2, Facilité 4, Écart 2, Cohérence 2) par Cohérence (3 contre 2), au même titre que l'idée n°25 (élaguer editor-chat) et l'idée n°26 déjà classées devant elle pour cette même raison : une Facilité plus faible pèse moins que servir un seuil `MISSION.md` plutôt que d'être en tension avec un principe déjà établi du produit. |

### 28. Journal de candidature (réalisations, culture d'entreprise, questions à poser) — 10/20
...
### 29. Remplacer le jeton d'authentification statique — 9/20
...
| **Total** | **9** | Apport 3, égal à l'idée n°30 (relecture humaine) mais Facilité plus favorable ici (2 contre 1) : implémenter un jeton de session est un chantier de code, contrairement à un partenariat ou une ligne de service humaine. Placée devant elle. Devant l'idée n°32 (`DEFAULT_STALE_DAYS`) par Apport (3 contre 1). |

### 30. Relecture de CV par un humain (service payant) — 9/20 — *sensible : touche
...
| Cohérence | 3 | Un contournement du rate limiting sur le login donnerait accès à tous les appels IA facturés de l'instance visée — touche indirectement le seuil « coût des appels externes » de `MISSION.md`, à l'image de l'idée n°41 déjà classée (compter/plafonner les appels IA). |
| **Total** | **9** | Apport 2, inférieur à l'idée n°30 (Apport 3) mais supérieur à l'idée n°32 (Apport 1) — placée entre les deux par ce seul critère. À traiter d'abord comme une vérification à faire par le propriétaire (accès infra requis) plutôt qu'un chantier de code prêt à lancer. |

### 32. Faire de `DEFAULT_STALE_DAYS` la seule source de vérité du délai de 30 jours — 9/20
...
### 33. Introduire une échelle `--font-size-*` et remplacer les 20 valeurs de `font-size` en dur — 8/20

Constat détaillé : `boucle/constats/2026-08-07-coherence-visuelle.md` §2.

| Critère | Note | Justification |
|---|---|---|
| Apport | 2 | Comme pour les rayons (idée n°27), aucune couleur ni contraste en jeu — cohérence visuelle pure. Signal aggravant : six des vingt valeurs sont au demi-pixel (9.5/10.5/11.5/12.5/13.5/14.5px), un pas qu'aucune échelle typographique voulue ne produit — plus probablement des ajustements ponctuels non documentés qu'un choix de design, mais reste un défaut de finition plutôt qu'un blocage. |
| Facilité | 2 | Vingt valeurs à répartir en six paliers, dans un fichier de 2896 lignes — surface plus large que les rayons (idée n°27, 19 valeurs en 4 paliers) et risque plus élevé : une taille de texte modifiée peut casser un retour à la ligne ou un alignement là où un rayon modifié ne change que l'angle d'un coin. Aucune mesure de régression visuelle automatisée dans le dépôt — vérification composant par composant à la main. |
...
### 35. Alertes sur de nouvelles offres correspondant au profil — 8/20 — *sensible : suppose
...
### 36. Décider du sort du chargement de Monaco depuis `cdn.jsdelivr.net` — 8/20
...
### 37. Étoffer le Content-Security-Policy — 8/20
...
| **Total** | **8** | Même profil (Apport 2, Facilité 3, Écart 1, Cohérence 2) que l'idée n°36 (Monaco CDN jsdelivr) : aucune des quatre notes ne les différencie. Conservée juste après elle, sans raison objective de l'y faire passer devant. |

### 38. Supprimer les fonctions mortes de `db.ts` et `completeJson` de `clients.ts` — 8/20
...
### 39. Capture d'offre native dans l'extension navigateur (éviter le copier-coller d'URL) — 8/20 — *estimation Facilité peu fiable*
...
### 40. Générateur de lettre de démission — 8/20
...
### 41. Compter/plafonner les appels IA comme les job-boards — 8/20 — *sensible : touche
...
### 42. Publier son CV en ligne (site personnel ou lien partageable) — 7/20 — *sensible :
...
| **Total** | **7** | Égalité de total avec l'idée n°43 (remplacer les sélecteurs CSS par `@mozilla/readability`) tranchée en sa faveur par Écart (2 contre 1) : celle-ci correspond à une capacité réellement comparée à la concurrence chez 2 produits sur 8, l'autre concerne un détail d'implémentation serveur invisible depuis l'extérieur d'un produit concurrent, le constat le dit lui-même « sans objet pour ce domaine ». Signalée explicitement sensible : sortir du 100 % local est un choix d'architecture, à trancher par le propriétaire. |

### 43. Remplacer les sélecteurs CSS faits main de `scrapeJobText` par `@mozilla/readability` — 7/20 — *sensible : ajout d'une dépendance npm importante, feu vert requis ; estimation Facilité peu fiable*
