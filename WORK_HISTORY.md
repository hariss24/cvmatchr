...
- **Pourquoi :** Le chantier est parti de trois URL rapportées par l'utilisateur. Deux constats l'ont réorienté. (1) Le plus utile n'était pas d'ajouter un connecteur mais de réparer la **découverte** : `jobs.groupe-psa.com` — l'exemple d'origine — n'est **ni** sous un domaine d'ATS connu, **ni** devinable depuis un nom SIRENE. Les deux voies existantes le ratent, et un connecteur Talentsoft seul n'aurait pas trouvé l'offre qui avait motivé le chantier. (2) L'index columnar permet de lire `url_host_name` **seul** : 0,8 Mo par fichier au lieu de 604 Mo, soit ~600 Mo pour l'univers des noms d'hôtes du web, contre 181 Go. Common Crawl ne documente que la voie Athena, payante et adossée à un compte AWS.
- **Mesures qui ont façonné le code :** `top` plafonne silencieusement à **1 000** offres (SPIE, EDF). `LCID=1036` est la **langue**, pas le pays : PSA publie Kenitra et Amsterdam, et des locataires entiers sont suisses (Chur) ou liechtensteinois (Vaduz). `estFrancais` **rejette** « SOCHAUX », « Poissy », « Marcoule » : la retenir vidait la moisson en silence — c'est le géocodage Base Adresse Nationale qui tranche le pays. Le lieu **n'est pas** la dernière `<category>` : Orange et Kronospan y mettent le type de contrat, la règle positionnelle inventait des villes nommées « CDI » et « Unbefristet ».
- **Deux pièges d'élimination silencieuse, attrapés par des tests avant la production :**
  1. `obtenirTextes` ne connaissait que cinq ATS, et `searchBoards` écarte toute offre sans texte. Sans `textesTalentsoft`, la source entière aurait été **invisible dans l'app** tout en pesant dans l'index.
  2. Le géocodeur nomme le département `departement`, tout l'aval lit `dept`. Comme les offres Talentsoft portent déjà des coordonnées, elles ne repassent pas par la conversion de `build-boards-offres.mjs` : **aucune n'aurait eu de département**, et les filtres région et département les auraient toutes écartées. Le raccord se fait désormais dans `talentsoft.mjs`, verrouillé par test. ⚠️ Le défaut était masqué par un faux géocodeur de test qui rendait `dept` — le test injecte maintenant la forme réelle.

  Troisième et quatrième occurrences du même mode de panne, après le tri alphabétique du 04/08 et la clé de tri manquante du 06/08.
- **Correction annexe dans `geo.mjs`, qui profite à TOUTES les sources :** le garde-fou exigeait que la commune trouvée soit *contenue* dans le libellé demandé. Les offres du site Stellantis de **Vélizy** écrivent « Vélizy », jamais « Vélizy-Villacoublay » : elles étaient perdues. On accepte désormais aussi le cas où la demande est le **premier mot entier** du nom trouvé. ⚠️ L'exigence du mot entier est ce qui empêche « Turin » de valider **Thurins** (près de Lyon) et « Amsterdam » les **Îles Saint-Paul et Nouvelle-Amsterdam** — deux cas réels du même flux, que la Base Adresse Nationale propose spontanément. Une longueur minimale de 5 caractères écarte « Bar ». Gain mesuré : PSA passe de 166 à **169** offres, sans laisser entrer une seule ville étrangère.
- **Vérification d'affichage et de filtres sur l'app réelle** (245 vraies offres injectées temporairement dans l'index local, `/api/jobs/search`) : titre nettoyé de sa référence interne, entreprise « Dassault Aviation » correctement déduite, source affichée « Talentsoft », lien exact, **texte de l'offre récupéré en direct depuis le flux**. Filtres : région Nouvelle-Aquitaine retient les offres de Mérignac, Île-de-France les écarte ; département 33 les retient ; rayon de 15 km autour de Bordeaux les retient, 2 km les écarte, Lille les écarte. ⚠️ L'affichage n'a pas pu être vu dans l'interface : la page Offres exige une session, et aucun compte n'a été créé.
- **Décision assumée :** le sondage **ne lit pas** `robots.txt`, sur demande explicite. Coût juridique chiffré et point d'insertion unique documentés dans `LIMITES.md` §8.3.
- **Validation réelle** (12 des 300 fichiers, soit 4 % du crawl) : 2 453 candidats en 27 s, sondés en 99 s, **3 boards Talentsoft, 202 offres françaises** — dont `jobs.groupe-psa.com` (166), plus deux découvertes sur domaine propre invisibles des anciennes méthodes (`carrieres.groupedci.com`, `recrutement.vinsrichard.fr`). PSA rend 166 offres sur 243 : Kenitra et Amsterdam écartés. Le locataire suisse RhB rend 0.
- **Infrastructure :** première dépendance à la racine (`hyparquet`, 1 paquet, 0 dépendance transitive) ; les trois workflows qui lancent les tests des scripts passent en Node 24 (ZSTD) et gagnent une étape `npm ci`.
- **Fichiers touchés :** `scripts/boards/{parquet,carrieres,talentsoft}.mjs` (+ tests), `scripts/boards/{offres,geo}.mjs` (+ tests), `scripts/build-boards-talentsoft.mjs`, `web/src/lib/jobs/boardsFr.ts` (+ test), `web/src/lib/jobs/boardsText.ts` (+ test), `package.json`, `.gitignore`, `.github/workflows/{boards-fr,boards-offres,web}.yml`, `PROJECT_INDEX.md`, `LIMITES.md`, `WORK_HISTORY.md`.
